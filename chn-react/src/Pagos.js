...
const Pagos = () => {
    ...
    const listaPagoPrestamoAll = async () => {
        const respuesta = await getFetchGET("http://localhost:8080/api/pago/getPagoPrestamosAllTable", "");
    ...
    const listaPagoPestamoByAprobadosAll = async (idPrestamosAprobado) => {
        const respuesta = await getFetchGET("http://localhost:8080/api/pago/getPagoBySolicitudAprobada/" + idPrestamosAprobado, "");
        console.log("respuesta", respuesta.data);
    ...
    const guardarPago = async () => {
        ...
        datosPago = {
            "idPrestamoAprobado": idSolicitudPrestamo,
            "montoPago": montoPago
        };
        ...
        const respuesta = await postFetchAll("http://localhost:8080/api/pago/saveOrUpdatePago", "POST", datosPago);
                            ...
                            return (
                                <tr id={"tr_" + index}>
                                    ...
                                    <td id={"td7_" + index} style={{ textAlign: 'center' }}>
                                        <GrTransaction fontSize={25} onClick={() => { toggle(); setIdSolicitudPrestamo(item.idPrestamoAprobado); setMontoSolicitado(item.montoSolicitado); setMontoAbono(item.montoPago); setMontoSaldo(item.saldo); }} />
                                    </td>
                                    <td id={"td8_" + index} style={{ textAlign: 'center' }}>
                                        <BsTicketDetailed fontSize={25} onClick={() => { togglePagosRealizados(); listaPagoPestamoByAprobadosAll(item.idPrestamoAprobado); }} />
                                    </td>
                                </tr>
                            )
                        })
